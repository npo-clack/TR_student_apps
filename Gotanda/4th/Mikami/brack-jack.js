...
if (newResult) {
  newResult.textContent = sumHand(hands1);
} else {
...
stand.addEventListener("click", () => {
  ...
  // 手札が21以上かを判定
  isBurst(hands2, dealerResult, "Dealer");

  // 勝敗判定
  if (sumHand(hands2) <= 21) {
    if (sumHand(hands1) > sumHand(hands2)) {
      result.textContent = `${sumHand(hands1)} : PlayerWIN!!`
      dealerResult.textContent = `${sumHand(hands2)} : DealerLOSE...`
      NoneBtn()
    } else if (sumHand(hands2) > sumHand(hands1)) {
      result.textContent = `${sumHand(hands1)} : PlayerLOSE...`
      dealerResult.textContent = `${sumHand(hands2)} : DealerWIN!!`
      NoneBtn()
    } else {
